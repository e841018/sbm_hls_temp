...
void print_J(float J[N][N]) {
    ...
    for (int i = 0; i < N; i++) {
        // std::cout << "    (" << std::setw(2) << i / n << ',' << std::setw(2)
        // << i % n << ")| ";
...
void print_h(float h[N]) {
    ...
    std::cout << "h =          ";
    for (int i = 0; i < N; i++) std::cout << std::setw(7) << h[i] << " ";
    std::cout << '\n';
}
...
void print_x_p(float x[Nb], float p[Nb]) {
    output() << "x | ";
    for (int i = 0; i < Nb; i++) output() << std::setw(7) << x[i] << " | ";
    output() << '\n';

    output() << "p | ";
    for (int i = 0; i < Nb; i++) output() << std::setw(7) << p[i] << " | ";
    output() << '\n';
}
#endif
...
*/
void top(float J[N][N], float h[N], float x_init[N], float p_init[N],
         bool spin[N]) {
...
#pragma HLS INTERFACE m_axi port = x_init offset = slave
#pragma HLS INTERFACE m_axi port = p_init offset = slave
#pragma HLS INTERFACE m_axi port = J offset = slave
#pragma HLS INTERFACE m_axi port = h offset = slave
    MMTE blocks[Pb];
    float std_of_J = sd(J);
    ...
    for (int b = 0; b < Pb; b++) {
        ...
        blocks[b].init(J + offset, h + offset, x_init + offset, p_init + offset,
                       Delta_t_times_gamma0);
    }
    SBM(J, h, blocks, spin);
}
...
/*
Simulated bifurcation machine for finding an approximate solution of an Ising
model of the form: \min_s \left( - \frac{1}{2} \sum_{i=0}^{N-1} \sum_{j=0}^{N-1}
{J_{ij} s_i s_j} + \sum_{i=0}^{N-1} {h_i s_i} \right)
...
void SBM(float J[N][N], float h[N], MMTE blocks[Pb], bool spin[N]) {
    ...
    for (int t = 0; t < N_step; t++) {
        ...
        for (int i = 0; i < N; i++) spin[i] = x_a[i] > 0;  // spin = sign(x)
...
#endif
    for (int b = 0; b < Pb; b++) print_x_p(blocks[b].x_sub, blocks[b].p_sub);
#ifdef LOG_X_P
    log_file.close();
#endif
#endif
...
*/
void MMTE::init(float J_sub[Nb][N], float h_sub[Nb], float x_sub_init[Nb],
                float p_sub_init[Nb], float Delta_t_times_gamma0) {
    for (int i = 0; i < Nb; i++) {
        for (int j = 0; j < N; j++) {
...
void MMTE::update_alpha_eta(int t) {
    ...
    // eta(t) is stated to be dependent on alpha(t) in [2], but not presents in
    // [1]
...
inline float MMTE::FX(float x_sub_i, float h_sub_i) {
    float delta_p =
        delta_t * (-(alpha0 - alpha + beta0 * x_sub_i * x_sub_i) * x_sub_i -
                   eta * h_sub_i);
    return delta_p;
}
